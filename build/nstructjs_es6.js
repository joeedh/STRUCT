 ...
 *  (one bulk copy, no per-element work). Wire bytes are unreplaced; the field
...
class StructTStructField extends StructFieldType {
    static pack(manager, data, val, obj, field, type) {
        ...
        if (valCtor.structName !== type.data && val instanceof cls) {
            stt = manager.get_struct(valCtor.structName);
        }
    ...
    static toJSON(manager, val, obj, field, type) {
        ...
        const valObj = val;
        const valCtor = valObj.constructor;
        let stt = manager.get_struct(valCtor.structName);
...
class StructStaticArrayField extends StructFieldType {
    static pack(manager, data, val, obj, field, type) {
        ...
        if (!debug &&
            !useEnv &&
            arr.length >= d.size &&
            packPrimitiveBulk(data, d.type.type, arr, d.size)) {
            return;
        }
    ...
    parse_structs(buf, defined_classes) {
        ...
        while (!struct_parse.at_end()) {
            ...
            if (!(stt.name in clsmap)) {
                ...
                dummy.STRUCT = STRUCT.fmt_struct(stt, undefined, undefined, undefined, true);
...
export { BinWriter, JSONError, STABLE_ID_BASE, STABLE_ID_LIMIT, STRUCT, _truncateDollarSign, struct_binpack as binpack, consoleLogger, deriveStructManager, struct_filehelper as filehelper, formatJSON, getEndian, inherit, inlineRegister, isRegistered, manager, struct_parser as parser, struct_parseutil as parseutil, readJSON, readObject, register, setAllowOverriding, setDebugMode, setEndian, setTruncateDollarSign, setWarningMode, stableStructId, truncateDollarSign, struct_typesystem as typesystem, unpack_context, unregister, validateJSON, validateStructs, writeJSON, writeObject, write_scripts };
